      ...
      child: Row(
        ...
        children: [
          ...
          SizedBox(
            width: AppTheme.operationsHeaderItemWidthNormal,
            child: Text(
              '${(operation['net_weight'] ?? 0.0).toString()}',
...
class OperationDetailsEntry extends StatelessWidget {
  final Map<String, dynamic> operation;
  final Function onPressed;

  const OperationDetailsEntry(
      {super.key, required this.operation, required this.onPressed});

  @override
  Widget build(BuildContext context) {
    final l11n = AppLocalizations.of(context)!;
    final nf = l11n.notAvailable;
    return Container(
      key: ValueKey(
          'operation_${operation['id'] ?? operation['tab_id'] ?? DateTime.now().millisecondsSinceEpoch}'),
      padding: const EdgeInsets.all(12),
      ...
      child: Row(
        ...
        children: [
          SizedBox(
              ...
              child: Align(
                alignment: Alignment.centerRight,
                child: Text('${operation['id'] ?? operation['tab_id'] ?? nf}'),
              )),
          SizedBox(
            width: AppTheme.operationsHeaderItemWidthNormal,
            child: Text(_formatDateTime(operation['created_at'] ?? nf)),
          ),
          SizedBox(
            width: AppTheme.operationsHeaderItemWidthNormal,
            child: Text(operation['truck_plate'] ?? nf),
          ),
          SizedBox(
            width: AppTheme.operationsHeaderItemWidthNormal,
            child: Text(operation['driver_name'] ?? nf),
          ),
          SizedBox(
              width: AppTheme.operationsHeaderItemWidthNormal,
              child: Text(operation['client'] ?? nf)),
          SizedBox(
              width: AppTheme.operationsHeaderItemWidthNormal,
              child: Text(operation['supplier'] ?? nf)),
          SizedBox(
              width: AppTheme.operationsHeaderItemWidthNormal,
              child: Text(_getMaterialName(operation))),
          SizedBox(
          ...
          SizedBox(
            width: AppTheme.operationsHeaderItemWidthNormal,
            child: Text(
              '${(operation['empty_weight'] ?? 0.0).toString()}',
              textAlign: TextAlign.center,
            ),
          ),
          SizedBox(
            width: AppTheme.operationsHeaderItemWidthNormal,
            child: Text(
              '${(operation['gross_weight'] ?? 0.0).toString()}',
              textAlign: TextAlign.center,
            ),
          ),
          SizedBox(
            width: AppTheme.operationsHeaderItemWidthNormal,
            child: Text(
              '${(operation['kilo_price'] ?? 0.0).toString()}',
              textAlign: TextAlign.center,
            ),
          ),
          SizedBox(
            width: AppTheme.operationsHeaderItemWidthNormal,
            child: Text(
              '${(operation['total_price'] ?? 0.0).toString()}',
              textAlign: TextAlign.center,
            ),
          ),
        ],
