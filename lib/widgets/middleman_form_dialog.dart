...
import 'package:weighing_system/providers/tabs_provider.dart';
import 'package:weighing_system/utils/arabic_normalize.dart';
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      if (widget.isEditing) {
        ...
        bool hasNameChanged = client.name != _nameController.text.trim();

        final success = await clientProvider.updateClient(updatedClient);
        if (success) {
          //TODO change all records
          if (hasNameChanged) {
            await context
                .read<TabsProvider>()
                .updateTabsWith('client', client.name, updatedClient.name);
          }
